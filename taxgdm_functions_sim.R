formatsitepair_tax <- function (bioData, bioFormat, dist = "bray", abundance = FALSE, 
                                  siteColumn = NULL, XColumn, YColumn, sppColumn = NULL, abundColumn = NULL, 
                                  sppFilter = 0, predData, distPreds = NULL, weightType = "equal", 
                                  custWeights = NULL, sampleSites = 1, verbose = FALSE) 
{
  ...
  if (bioFormat == 1 | bioFormat == 2) {
    ...
    if (abundance == F) {
      ...
      distData <- beta.pair(sppData, index.family = "sorensen")$beta.sim
    }
    else {
      sppData[is.na(sppData)] <- 0
      distData <- beta.pair(sppData, index.family = "sorensen")$beta.sim}
